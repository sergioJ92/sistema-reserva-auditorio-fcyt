...
function obtenerReservasAcademicas($anio, $gestion) {
    ...
    $consulta .= "FROM reserva as r, reserva_academica as ra, actividad as a, contenido as c, cronograma_academico as ca, asunto as asu, materia as ma, responsable_reserva as rr, usuario as u ";
    $consulta .= "WHERE r.id_reserva = ra.id_reserva AND a.id_contenido = ra.id_contenido AND c.id_contenido = a.id_contenido AND c.anio = ca.anio AND asu.id_asunto = ra.id_asunto AND c.gestion = ca.gestion AND ma.codigo_materia = ra.codigo_materia AND ra.id_reserva = rr.id_reserva AND rr.nombre_usuario = u.nombre_usuario  AND ca.anio = '$anio' AND ca.gestion = '$gestion'";
